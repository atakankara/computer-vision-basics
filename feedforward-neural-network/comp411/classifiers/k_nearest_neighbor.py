...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        Inputs:
        - X: A numpy array of shape (num_test, D) containing test data. shape 500, 3072
    ...
    def compute_L2_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            # dists[i, :] = np.sqrt(np.sum((X[i] - self.X_train[i])**2))
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        x_sqrd = np.sum(self.X_train **2, axis = 1).reshape(1, -1)
        y_sqrd = np.sum(X **2, axis = 1).reshape(-1, 1)
        xy = X @ self.X_train.T
        dists = np.sqrt(x_sqrd + y_sqrd - 2*xy)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                nominator = np.dot(X[i], self.X_train[j])
                denominator = np.sqrt(np.dot(X[i], X[i]) * np.dot(self.X_train[j], self.X_train[j]))
                dists[i, j] = nominator/denominator
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i, :] = np.dot(self.X_train, X[i]) / np.sqrt(np.dot(X[i], X[i]) * np.sum(self.X_train * self.X_train, axis=1))
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        x_norm = np.sqrt(np.sum(self.X_train **2, axis = 1)).reshape(1, -1)
        y_norm = np.sqrt(np.sum(X **2, axis = 1)).reshape(1, -1)
        denominator = np.matmul(y_norm.T, x_norm)
        nominator = np.matmul(X, self.X_train.T)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #k_neighbors = np.argsort(dists, axis = 1)[:, :k]
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #take mode of the row to get the majority voting
