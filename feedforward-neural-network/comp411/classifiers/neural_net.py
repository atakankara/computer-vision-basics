...
class FourLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        grads['b4'] = np.sum(softmax_gradient, axis=0) / scores.shape[0]
        grads['W4'] = ((r3.T @ softmax_gradient) / scores.shape[0]) + (2 * reg * W4)

        temp = softmax_gradient @ W4.T * (r3 > 0)
        grads['b3'] = np.sum(temp, axis=0) / scores.shape[0]
        grads['W3'] = ((r2.T @ temp) / scores.shape[0]) + (2 * reg * W3)

        grads['b2'] = np.sum(temp @ W3.T * (r2 > 0), axis=0) / scores.shape[0]
        grads['W2'] = ((r1.T @ (temp @ W3.T * (r2 > 0))) / scores.shape[0]) + (2 * reg * W2)

        grads['b1'] = np.sum((temp @ W3.T * (r2 > 0)) @ W2.T * (r1 > 0), axis=0) / scores.shape[0]
        grads['W1'] = ((X.T @ ((temp @ W3.T * (r2 > 0)) @ W2.T * (r1 > 0))) / scores.shape[0]) + (2 * reg * W1)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            indices = random.sample(range(num_train), min(batch_size, num_train))
            X_batch = X[indices]
            y_batch = y[indices]
